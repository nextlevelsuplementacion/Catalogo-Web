      ...
      <div className="max-w-[1440px] mx-auto flex items-center justify-between gap-4">
        <div className="flex items-center gap-8">
          ...
          <nav className="hidden lg:flex items-center gap-6">
            <button 
              onClick={() => setPage('catalog')}
              className={`flex items-center gap-2 text-sm font-bold uppercase tracking-widest transition-colors ${page === 'catalog' ? 'text-primary' : 'text-slate-500 hover:text-primary'}`}
            >
            ...
            <button 
              ...
              className={`flex items-center gap-2 text-sm font-bold uppercase tracking-widest transition-colors ${page === 'points' ? 'text-primary' : 'text-slate-500 hover:text-primary'}`}
            >
        ...
        <div className="flex flex-1 justify-end items-center gap-4">
          <div className="hidden md:flex relative w-full max-w-xs">
            ...
            <input 
              ...
              placeholder="Buscar productos (ej. Whey, Star)..."
              value={searchQuery}
          ...
          <div className="flex gap-2">
            <button 
              onClick={() => setPage('catalog')}
              className={`lg:hidden size-10 flex items-center justify-center rounded-full transition-all ${page === 'catalog' ? 'bg-primary text-black shadow-lg shadow-primary/30 scale-110' : 'bg-slate-100 dark:bg-zinc-900 hover:bg-primary/10'}`}
            >
              <LayoutGrid className="size-5" />
            </button>
            <button 
              onClick={() => setPage('points')}
              className={`lg:hidden size-10 flex items-center justify-center rounded-full transition-all ${page === 'points' ? 'bg-primary text-black shadow-lg shadow-primary/30 scale-110' : 'bg-slate-100 dark:bg-zinc-900 hover:bg-primary/10'}`}
            >
